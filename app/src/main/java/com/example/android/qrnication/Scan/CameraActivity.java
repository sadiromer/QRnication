...
import com.example.android.qrnication.Engine.PlanarYUVLuminanceSource;
import com.example.android.qrnication.MainActivity;
...
public class CameraActivity extends Activity implements SurfaceHolder.Callback  {
    ...
    public ArrayList<String>StringOutput = new ArrayList<String>();
    //----------------------------------------------------------------------------------------------



    private void closeActivity(boolean abort, ArrayList<BinaryBitmap> binBitmapArray, ArrayList<String> output){
        exiting= true;
        handler.removeCallbacks(previewRunnable);
        stopAutofocus();
        camera.stopPreview();
        previewRunning = false;
        Intent intent = new Intent(CameraActivity.this, ScanContinuousActivity.class);
...
/*
        if (!abort) {
            ...
            //intent.putExtra("FILES_TO_SEND", output);
            intent.putExtra("FILES_TO_SEND", test);
            startActivity(intent);
        }
        */
        finish();
    ...
    private Camera.PreviewCallback previewCallback = new Camera.PreviewCallback() {
        ...
        public void onPreviewFrame(byte[] data, Camera camera) {
            ...
            if(!extracting){
                ...
                //once the start is detected it goes to this block, decodes the QR code and adds it
                //to StringResults
                //Data should be taken from the StringResults and stored to stringOutput and the
                //following conditions should be considered
                // 1)If frame detects start again
                // 2)If its not start
                // 3)If frames are equal
                // 4)If frames are not
                // 5) If frame is "end"
            }else{
                ...
                try {
                    ...
                    /*
                    if (result.toString().equals("START") && decodingPhase == 1){
                        decodingPhase = 2;
                    }

                    if (result.toString().equals("START") && decodingPhase == 2){
                        decodingPhase = 2;
                    }

                    if (!(result.toString().equals("START")) && decodingPhase == 2){
                            StringOutput.add(result.toString());
                            System.out.println("\n result.toString(): of 2 >>" + StringOutput + "<<");
                    }
*/
